...
<div class="rightColumn" style="margin-left:250px">
    ...
    <form method="POST" action="homeSupplies.php">
  ...
  transition: box-shadow 0.3s, border 0.3s;">
    ...
    $userId = $row['user_id'];
    echo "<a href='upload_homeSupplies.php?user_id=$userId' class='label label-success'>Upload an item</a><br><br><br>";

    if($mysqli->error){
        exit($mysqli->error);
    }

    $sqlItems = "SELECT * FROM HomeSupplies  ORDER BY item_id DESC";
    ...
    if(!$results_items){
        ...
        if ($_POST['searchItem'] == '') {
            ...
            while ($row = $results_items->fetch_array(MYSQLI_ASSOC)) {
                ...
                $sqlUsers = "SELECT * FROM Users, HomeSupplies
    ...
    ?></div> <?php
            ...
            $sqlSearchItems = "SELECT * FROM HomeSupplies WHERE item_name LIKE '%$searchResult%'  ORDER BY item_id DESC";
