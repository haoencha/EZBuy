...
<form class="form-horizontal" method="POST" action="upload_schoolSupplies.php?user_id= <?php echo $user_id?>" enctype="multipart/form-data">
        <fieldset>
...
if(isset($_POST['submit'])) {
    if (empty($_POST['productName'])) {
        exit("Please fill out the product name!");
    }
    if (empty($_POST['price'])) {
        exit("Please fill out the product price!");
    }
    if (empty($_POST['description'])) {
        exit("Please fill out the product description!");
    }
...
function saveItem($productName, $price, $description, $user_id, $name, $image){
    ...
    $sql = "INSERT INTO SchoolSupplies (item_name, price, description, image, image_name, user_info)
                VALUES ('$productName', '$price', '$description', '$image', '$name', $user_id)";
    $sqlAll = "INSERT INTO AllItems (item_name, price, description, image, image_name, user_info)
                VALUES ('$productName', '$price', '$description', '$image', '$name', $user_id)";
    $result = $mysqli->query($sql);
    $mysqli->query($sqlAll);

    if($result){
        echo "<br> <div style='text-align:center; font-size:25px'> Item uploaded!<a href='schoolSupplies.php' style=' margin-left:15px'><u>Back</u></a></div>";
    }else{
        echo "<br><div style='text-align:center; font-size:25px'>  Item not uploaded".$mysqli->error."
            <a href='schoolSupplies.php' style=' margin-left:15px'><u>Back</u></a></div>";
