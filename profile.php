...
<!--    </form>-->
    <?php
        ...
        $userId = $row['user_id'];
        echo "<a href='upload.php?user_id=$userId' class='label label-success'>Upload an item</a><br><br><br>";
        ...
        $sqlItems = "SELECT * FROM AllItems ORDER BY item_id DESC";
   ...
   $sqlItems = $sqlItems. " LIMIT $start_index, $results_per_page";
    ...
    if(!$results_items){
            ...
            if($_POST['searchItem']=='') {
                while ($row = $results_items->fetch_array(MYSQLI_ASSOC)) {
                    ...
                    $sqlUsers = "SELECT * FROM Users, AllItems
    ...
    if($page_num > $first_page) {
                ...
                $page_search_num = $_GET['pages'];
                $searchResult = $_POST['searchItem'];
                $sqlSearchItems = "SELECT * FROM AllItems WHERE item_name LIKE '%$searchResult%' ORDER BY item_id DESC";

                $results_SearchItems = $mysqli->query($sqlSearchItems);

                $num_search_results = $results_SearchItems->num_rows;

                // ceiling of Total # of results / # of results per page
                $last_search_page = ceil($num_search_results/ $results_per_page);

                if(empty($page_search_num)){
                    $page_search_num = $first_page;
                }else{
                    if($page_search_num < $first_page){
                        $page_search_num = $first_page;
                    }elseif($page_search_num > $last_search_page){
                        $page_search_num = $last_search_page;
                    }
                }
                $start_search_index = ($page_search_num-1) * $results_per_page;

                $sqlSearchItems = $sqlSearchItems. " LIMIT $start_search_index, $results_per_page";
...
<!---->
<!---->
<!--                        <a href="--><?php //echo $_SERVER['PHP_SELF'] . "?pages=$first_page" ?><!--">[<< First]</a>-->
<!---->
<!--                        <a href="--><?php //echo $_SERVER['PHP_SELF'] . "?pages=".($page_search_num - 1) ?><!--">[< Previous]</a>-->
<!--                        --><?php
//                    }
//                    ?>
<!---->
<!--                    --><?php //echo $page_search_num; ?>
<!---->
<!--                    --><?php
//                    if($page_search_num < $last_search_page) {
//                        ?>
<!---->
<!---->
<!--                        <a href="--><?php //echo $_SERVER['PHP_SELF'] . "?pages=" . ($page_search_num + 1) ?><!--">[Next >]</a>-->
<!--                        <a href="--><?php //echo $_SERVER['PHP_SELF'] . "?pages=$last_search_page" ?><!--">[Last >>]</a>-->
<!---->
<!--                        --><?php
//                    }
//                }
