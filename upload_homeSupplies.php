...
<form class="form-horizontal" method="POST" action="upload_homeSupplies.php?user_id= <?php echo $user_id?>" enctype="multipart/form-data">
        <fieldset>
...
function saveItem($productName, $price, $description, $user_id, $name, $image){
    ...
    $sql = "INSERT INTO HomeSupplies (item_name, price, description, image, image_name, user_info)
    ...
    if($result){
        echo "<br><div style='text-align:center; font-size:25px'> Item uploaded!<a href='homeSupplies.php' style=' margin-left:15px'><u>Back</u></a></div>";
    }else{
        echo "<br> <div style='text-align:center; font-size:25px'>Item not uploaded".$mysqli->error."
            <a href='homeSupplies.php' style=' margin-left:15px'><u>Back</u></a></div>";
