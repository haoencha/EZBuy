...
<div class="rightColumn" style="margin-left:250px">
    <form method="POST" action="entertainment.php">
  ...
  transition: box-shadow 0.3s, border 0.3s;">
    ...
    $userId = $row['user_id'];
    echo "<a href='upload_entertainment.php?user_id=$userId' class='label label-success'>Upload an item</a><br><br><br>";
    ...
    $sqlItems = "SELECT * FROM Entertainment  ORDER BY item_id DESC";
    ...
    if(!$results_items){
        ...
        if ($_POST['searchItem'] == '') {
            ...
            while ($row = $results_items->fetch_array(MYSQLI_ASSOC)) {
                ...
                $sqlUsers = "SELECT * FROM Users, Entertainment
    ...
    ?></div> <?php
            ...
            $sqlSearchItems = "SELECT * FROM Entertainment WHERE item_name LIKE '%$searchResult%'  ORDER BY item_id DESC";
            ...
            while ($row = $results_SearchItems->fetch_array(MYSQLI_ASSOC)) {
                ...
                $rowUser = $results_users->fetch_array(MYSQLI_ASSOC);
                echo "<strong> Seller Info: </strong><ul><li>Username: <u>" . $rowUser['username'] . "</u></li><li>Email: <u>
                       " . $rowUser['email'] . "</u></li></ul><form method='POST' action='favoriteInsert.php'>
