...
<div class="rightColumn" style="margin-left:250px">
    <form method="POST" action="schoolSupplies.php">
  ...
  transition: box-shadow 0.3s, border 0.3s;">
    ...
    $userId = $row['user_id'];
    echo "<a href='upload_schoolSupplies.php?user_id=$userId' class='label label-success'>Upload an item</a><br><br><br>";
    ...
    $sqlItems = "SELECT * FROM SchoolSupplies ORDER BY item_id DESC";
    ...
    if(!$results_items){
        ...
        if ($_POST['searchItem'] == '') {
            ...
            while ($row = $results_items->fetch_array(MYSQLI_ASSOC)) {
                echo "
                    <img height='300' src='data:image;base64," . $row['image'] . "'><br><br><strong>Product Name: </strong>" . $row['item_name'] . "<br>
                       <strong>Price: </strong>" . $row['price'] . "<br>
                       <strong> Description: </strong>" . $row['description'] . "<br>";
                $user_info = $row['user_info'];
                $sqlUsers = "SELECT * FROM Users, SchoolSupplies
    ...
    ?></div> <?php
            ...
            $sqlSearchItems = "SELECT * FROM SchoolSupplies WHERE item_name LIKE '%$searchResult%'  ORDER BY item_id DESC";
